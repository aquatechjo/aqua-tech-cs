...
const scopeServer = readFileSync(
  "src/lib/task-scope-server.ts",
  "utf8"
)
...
test("AD-03 is included in quality gates and the adoption roadmap", () => {
  assert.match(
    packageJson.scripts["test:unit"],
    /task-scope\.test\.ts/u
  )
  assert.match(
    packageJson.scripts["test:unit"],
    /design-system-tasks-adoption\.test\.ts/u
  )
