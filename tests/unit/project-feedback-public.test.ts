...
test('PROJ-09 validates opaque tokens and bounded public input', () => {
  ...
  assert.equal(
    publicFeedbackSubmissionSchema.safeParse({
      ...
      npsScore: 9,
      satisfactionScore: 5,
      feedbackSummary: 'تجربة واضحة وممتازة في جميع مراحل المشروع',
  ...
  assert.equal(
    publicFeedbackSubmissionSchema.safeParse({
      ...
      npsScore: 9,
      satisfactionScore: 5,
      feedbackSummary: 'تجربة واضحة وممتازة',
...
test('PROJ-09 stores only hashes and governs one transactional submission', () => {
  ...
  const migration = readFileSync(
    'prisma/migrations/20260809190000_proj_09_secure_client_feedback/migration.sql',
    'utf8',
  );
