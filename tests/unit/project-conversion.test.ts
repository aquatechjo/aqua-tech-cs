...
test('accepted proposal conversion binds status, version, client hash, and authority', () => {
  assert.deepEqual(acceptedProposalConversionIssues(acceptedSnapshot), []);
  assert.match(
    acceptedProposalConversionIssues({
      ...acceptedSnapshot,
...
test('client resolution reuses one unique client and blocks ambiguous matches', () => {
  ...
  assert.deepEqual(resolveClientCandidateIds(['client-1', ' client-1 ']), {
    status: 'MATCHED',
    clientId: 'client-1',
  });
  assert.deepEqual(resolveClientCandidateIds(['client-1', 'client-2']), {
    status: 'AMBIGUOUS',
    clientId: null,
  });
});
...
test('PROJ-01 route is tenant-scoped, locked, hash-verified, and replay-safe', () => {
  const route = readFileSync(
    resolve(process.cwd(), 'src/app/api/sales/opportunities/[id]/convert/route.ts'),
    'utf8',
  );
  const acceptedBranchStart = route.indexOf('if (opportunity.proposalWorkspace)');
  const legacyBranchStart = route.lastIndexOf('if (opportunity.stage === "LOST")');
  const acceptedBranch = route.slice(acceptedBranchStart, legacyBranchStart);
  ...
  assert.match(acceptedBranch, /proposalClientContentHash\(acceptedContent\.data\)/);
...
test('accepted proposals create planning workflows without automatic schedules or owners', () => {
  ...
  const projectPage = readFileSync(
    resolve(process.cwd(), 'src/app/dashboard/projects/[id]/ProjectExecutionClient.tsx'),
    'utf8',
  );

  assert.match(workflowServer, /return ["']NOT_STARTED["']/);
  assert.match(workflowServer, /dueDate: project\.startDate[\s\S]+: null/);
...
test('PROJ-01 workspace uses canonical confirmation and exposes the converted project', () => {
  const workspace = readFileSync(
    resolve(process.cwd(), 'src/app/dashboard/discovery/[id]/proposal/ProposalWorkspaceClient.tsx'),
    'utf8',
  );
  const queue = readFileSync(
    resolve(process.cwd(), 'src/app/dashboard/proposals/page.tsx'),
    'utf8',
  );
