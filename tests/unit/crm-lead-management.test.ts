...
test("CRM-02 applies server-side access, canonical workflow UI, and safe confirmations", () => {
  ...
  assert.match(client, /serviceRequestStatusLabel/)
  assert.match(client, /trailingIcon=\{<ArrowLeft \/>\}/)
  assert.match(client, /إضافة عميل محتمل/)
  assert.doesNotMatch(client, /إضافة Lead/u)
  assert.doesNotMatch(client, /Leads مفتوحة/u)
  assert.doesNotMatch(client, /ArrowUpRight/u)
