...
const dashboardCss = readFileSync('src/styles/aqua-dashboard.css', 'utf8');
const crmSalesCss = readFileSync('src/styles/aqua-crm-sales.css', 'utf8');
const discoveryProposalCss = readFileSync('src/styles/aqua-discovery-proposal.css', 'utf8');
...
const leadsPage = readFileSync('src/app/dashboard/leads/LeadsClient.tsx', 'utf8');
const salesPage = readFileSync('src/app/dashboard/sales/SalesPipelineClient.tsx', 'utf8');
...
test('AD-02.6 dashboard uses canonical Aqua components without alert duplication', () => {
  for (const component of ['AquaBadge', 'AquaCard', 'AquaDataPanel', 'AquaLinkButton']) {
...
test('UI-18 keeps the dashboard compact and operational', () => {
  for (const token of [
    ...
    assert.match(dashboardCss, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'u'));
  }
...
test('UI-09 applies one CRM and Sales workspace contract', () => {
  ...
  for (const token of [
    ...
    assert.match(crmSalesCss, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'u'));
  }
});
...
test('AD-01 is recorded in the adoption roadmap', () => {
