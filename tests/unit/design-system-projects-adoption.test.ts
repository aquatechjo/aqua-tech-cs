...
const page = readFileSync('src/app/dashboard/projects/page.tsx', 'utf8');
const detailPage = readFileSync('src/app/dashboard/projects/[id]/page.tsx', 'utf8');
const client = readFileSync('src/app/dashboard/projects/ProjectsClient.tsx', 'utf8');
const executionClient = readFileSync(
  'src/app/dashboard/projects/[id]/ProjectExecutionClient.tsx',
  'utf8',
);
const css = readFileSync('src/app/dashboard/projects/Projects.module.css', 'utf8');
const executionCss = readFileSync(
  'src/app/dashboard/projects/[id]/ProjectExecution.module.css',
  'utf8',
);
const ui11Css = readFileSync('src/styles/aqua-project-execution.css', 'utf8');
...
const listApi = readFileSync('src/app/api/projects/route.ts', 'utf8');
const detailApi = readFileSync('src/app/api/projects/[id]/route.ts', 'utf8');
const executionApi = readFileSync('src/app/api/projects/[id]/execution/route.ts', 'utf8');
const memberApi = readFileSync('src/app/api/projects/[id]/members/route.ts', 'utf8');
const participantApi = readFileSync('src/app/api/tasks/[id]/participants/route.ts', 'utf8');
const aquaInput = readFileSync('src/components/aqua/AquaInput.tsx', 'utf8');
const aquaSelect = readFileSync('src/components/aqua/AquaSelect.tsx', 'utf8');
...
test('AD-04 Projects uses canonical data and workflow components', () => {
  ...
  assert.equal(
    (client.match(/className=\{styles\.dateField\} data-aqua-span="6"/gu) ?? []).length,
    2,
  );
...
test('AD-04 separates metadata execution finance and assignment permissions', () => {
  ...
  assert.doesNotMatch(detailApi, /requireProjectExecutionManager\(user, id\)/u);
  assert.match(listApi, /scope\.canViewProjectBudgets/u);
  assert.match(detailApi, /scope\.canViewProjectBudgets/u);
  assert.match(executionApi, /projectScope\.canViewProjectBudgets/u);
  assert.match(memberApi, /canAssignTaskTo\(scope, employee\.userId\)/u);
  assert.match(memberApi, /PROJECT_MEMBER_SCOPE_FORBIDDEN/u);
  assert.match(participantApi, /TASK_PARTICIPANT_SCOPE_FORBIDDEN/u);
});

test('AD-04 supports compact field sizes without native size conflicts', () => {
  assert.match(aquaInput, /Omit<[\s\S]*InputHTMLAttributes<HTMLInputElement>[\s\S]*"size"/u);
  assert.match(aquaSelect, /Omit<[\s\S]*SelectHTMLAttributes<HTMLSelectElement>[\s\S]*"size"/u);
});
...
test('AD-04 styling covers responsive logical and reduced-motion behavior', () => {
  ...
  assert.match(css, /\.projectModal\s*\{[\s\S]*block-size:\s*min\(900px, 90vh\);/u);
...
test('UI-11 applies one Projects and execution workspace contract', () => {
  ...
  for (const contract of [
    ...
    assert.match(ui11Css, new RegExp(contract.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&'), 'u'));
  }
});

test('AD-04 is recorded in the adoption roadmap', () => {
