...
import type { ProjectPriority, ProjectStatus, WorkflowStatus } from '@/generated/prisma/enums';
...
export function projectStatusToWorkflowStatus(status: ProjectStatus): WorkflowStatus {
  if (status === 'IN_PROGRESS') return 'ACTIVE';
  if (status === 'AT_RISK') return 'ACTIVE';
  if (status === 'IN_REVIEW') return 'ACTIVE';
  if (status === 'READY_FOR_DELIVERY') return 'ACTIVE';
...
async function resolveTemplate(
  ...
  if (!template) {
    throw new ApiError('لا يوجد قالب سير عمل مفعّل لهذه الشركة', 409, 'WORKFLOW_TEMPLATE_REQUIRED');
  }
...
export async function createProjectWithWorkflow(
  ...
  const completedAt =
    input.project.status === 'COMPLETED' ? (input.project.completedAt ?? new Date()) : null;
  ...
  await tx.projectReadiness.create({
    data: {
      ...
      contractRequired: input.readiness?.contractRequired ?? false,
      paymentRequired: input.readiness?.paymentRequired ?? false,
      requiredPaymentAmount: input.readiness?.requiredPaymentAmount ?? null,
      currency: input.readiness?.currency ?? input.project.currency,
    },
  });
    ...
    const phase = await tx.projectPhase.create({
      data: {
        ...
        startDate: project.startDate
          ...
          : null,
        dueDate: project.startDate ? addWorkflowDays(project.startDate, stage.dueOffsetDays) : null,
  ...
  for (const approval of definition.approvals) {
    await tx.projectWorkflowApproval.create({
      data: {
        ...
        phaseId: approval.stageCode ? phaseIds.get(approval.stageCode) : null,
        taskId: approval.taskCode ? taskIds.get(approval.taskCode) : null,
        code: approval.code,
        name: approval.name,
        gate: approval.gate,
