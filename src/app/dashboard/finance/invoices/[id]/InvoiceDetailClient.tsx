...
type Invoice = {
  ...
  publicIssuedAt: string | null;
  publicRevokedAt: string | null;
  publicFirstViewedAt: string | null;
...
export default function InvoiceDetailClient({
  ...
  const [collectionNextAction, setCollectionNextAction] = useState(
    invoice.collectionNextAction ?? '',
  );
  const [collectionNextActionAt, setCollectionNextActionAt] = useState(
    invoice.collectionNextActionAt?.slice(0, 10) ?? '',
  );
  const [collectionPromiseDate, setCollectionPromiseDate] = useState(
    invoice.collectionPromiseDate?.slice(0, 10) ?? '',
  );
  ...
  const portalUrl =
    portalPath && typeof window !== 'undefined'
      ...
      : portalPath;
  const linkUrl =
    linkPath && typeof window !== 'undefined' ? `${window.location.origin}${linkPath}` : linkPath;
    ...
    const saved = await mutate(
      {
        action: 'ISSUE',
        dueDate: dueDate || null,
        ...(invoice.contractAmendment ? { issueReference, taxDecision } : {}),
      },
      'issue',
    );
  ...
  async function managePortal(action: 'ISSUE' | 'REVOKE') {
    ...
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/invoices/${invoice.id}/portal`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(
          action === 'ISSUE' ? { action, validDays: Number(portalDays) } : { action },
        ),
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر إدارة رابط الفاتورة');
        return;
      }
      setPortalPath(payload.data.path ?? '');
      setSuccess(
        action === 'ISSUE'
          ? 'تم إصدار رابط جديد. انسخه الآن؛ لا يُخزن الرمز بصورته الأصلية.'
          : 'تم إلغاء رابط الفاتورة',
      );
      router.refresh();
    } catch {
      setError('تعذر الاتصال بالخادم');
  ...
  async function manageLink(action: 'ISSUE' | 'REVOKE') {
    setBusy(`link-${action.toLowerCase()}`);
    setError('');
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/invoices/${invoice.id}/public-link`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(
          action === 'ISSUE' ? { action, validDays: Number(linkDays) } : { action },
        ),
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر إدارة الرابط العام للفاتورة');
        return;
      }
      setLinkPath(payload.data.path ?? '');
      setSuccess(
        action === 'ISSUE'
          ? 'تم إصدار رابط جديد. انسخه الآن؛ لا يُخزن الرمز بصورته الأصلية.'
          : 'تم إلغاء الرابط العام للفاتورة',
      );
      router.refresh();
    } catch {
      setError('تعذر الاتصال بالخادم');
  ...
  async function sendPaymentReminder() {
    ...
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/invoices/${invoice.id}/portal/reminder`, {
        method: 'POST',
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر إرسال تذكير الدفع');
        return;
      }
  ...
  async function updateReminderSchedule(enabled: boolean) {
    ...
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/invoices/${invoice.id}/portal/reminder-schedule`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر تحديث جدولة التذكيرات');
        return;
      }
  ...
  async function deliverPaymentReceipt() {
    ...
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/payments/${receiptPaymentId}/receipt/delivery`, {
        method: 'POST',
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر إرسال إيصال الدفعة');
        return;
      }
  ...
  async function saveCollection(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSuccess('');
    try {
      const response = await fetch(`/api/finance/invoices/${invoice.id}/collection`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: collectionStatus,
          ownerId: collectionOwnerId || null,
          nextAction: collectionNextAction || null,
          nextActionAt: collectionNextActionAt || null,
          promiseDate: collectionPromiseDate || null,
          notes: collectionNotes || null,
        }),
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) {
        setError(payload.message || 'تعذر تحديث متابعة التحصيل');
        return;
      }
    ...
    <div className="aqua-invoice-detail-page">
      ...
      <div className="aqua-finance-actions aqua-finance-actions--split d-print-none">
        <div className="d-flex flex-wrap gap-2">
          <Link className="btn btn-outline-info" href="/dashboard/finance/invoices">
            رجوع للفواتير
          </Link>
        ...
        <div className="d-flex flex-wrap gap-2">
          <button className="btn btn-outline-light" type="button" onClick={() => window.print()}>
            طباعة
          </button>
          {invoice.contractAmendment &&
          ['ISSUED', 'PARTIALLY_PAID', 'PAID'].includes(invoice.status) ? (
            <Link
              ...
              href={`/invoice-document/${invoice.id}`}
              target="_blank"
            >
              مستند فاتورة الملحق
            </Link>
      ...
      <div className="aqua-card p-4">
        <div className="row g-4">
          <div className="col-12 col-lg-8">
            ...
            <h2 className="h3 fw-black mt-2" dir="ltr">
          ...
          <div className="col-12 col-lg-4 text-lg-start">
            ...
            <div className="small aqua-muted mt-1" dir="ltr">
      ...
      {invoice.contractAmendment ? (
        ...
        >
          القيمة الأساسية المعتمدة{' '}
          {money(invoice.contractAmendment.financialAmount, invoice.currency)} ثابتة دون خصم أو
          تعديل بنود.
        ...
        <div className="aqua-card p-4 d-print-none">
          ...
          {invoice.contractAmendment.invoiceDeliverySentAt ? (
            <AquaAlert variant="success" title="تم إرسال الفاتورة">
              أُرسلت إلى {invoice.contractAmendment.invoiceDeliveryRecipientEmail} بمرجع{' '}
              {invoice.contractAmendment.invoiceDeliveryReference}.
            </AquaAlert>
          ) : (
            <>
      ...
      canManage ? (
        <div className="aqua-card p-4 d-print-none">
          ...
          <p className="aqua-muted">
            أنشئ رابطًا عشوائيًا محدود الصلاحية. إصدار رابط جديد يلغي الرابط السابق فورًا.
          </p>
          {invoice.contractAmendment.invoicePortalIssuedAt &&
          !invoice.contractAmendment.invoicePortalRevokedAt ? (
            <AquaAlert variant="info" title="يوجد رابط مُصدر">
              ينتهي في {dateOnly(invoice.contractAmendment.invoicePortalExpiresAt)} — عدد المشاهدات{' '}
              {invoice.contractAmendment.invoicePortalViewCount}.
              {invoice.contractAmendment.invoicePortalFirstViewedAt
                ? ` أول مشاهدة: ${dateOnly(invoice.contractAmendment.invoicePortalFirstViewedAt)}.`
                : ' لم يُفتح بعد.'}
            </AquaAlert>
          ) : null}
          {portalPath ? (
          ...
          <div className="d-flex flex-wrap align-items-end gap-2 mt-3">
            <div>
              ...
              <input
                ...
                max="30"
                value={portalDays}
                onChange={(event) => setPortalDays(event.target.value)}
              />
            </div>
            <button
              className="btn btn-info fw-bold"
              type="button"
              disabled={Boolean(busy) || Number(portalDays) < 1 || Number(portalDays) > 30}
              onClick={() => managePortal('ISSUE')}
            >
              {busy === 'portal-issue' ? 'جارٍ الإصدار...' : 'إصدار / تدوير الرابط'}
            </button>
            {invoice.contractAmendment.invoicePortalIssuedAt &&
            !invoice.contractAmendment.invoicePortalRevokedAt ? (
              <button
              ...
              >
                {busy === 'portal-revoke' ? 'جارٍ الإلغاء...' : 'إلغاء الرابط'}
              </button>
            ) : null}
          </div>
          <div className="aqua-card-soft p-3 mt-3">
            ...
            <p className="small aqua-muted">
              يُفعّل الرابط الجديد فقط بعد قبول البريد، ويبقى الرابط السابق فعالًا إذا فشلت
              المحاولة.
            </p>
            {invoice.contractAmendment.invoicePortalDeliveryFailedAt ? (
              <AquaAlert variant="warning" title="فشلت المحاولة السابقة">
                تم توثيق الفشل ويمكن إعادة المحاولة دون تعطيل الرابط السابق.
              </AquaAlert>
            ) : null}
            {invoice.contractAmendment.invoicePortalDeliverySentAt ? (
              <AquaAlert variant="success" title="آخر إرسال ناجح">
                أُرسل الرابط إلى {invoice.contractAmendment.invoicePortalDeliveryRecipientEmail} —
                المحاولات: {invoice.contractAmendment.invoicePortalDeliveryAttemptCount}.
              </AquaAlert>
            ) : null}
            <div className="row g-2 align-items-end">
              <div className="col-12 col-md-5">
                ...
                <input
                  className="form-control"
                  maxLength={120}
                  value={portalRecipientName}
                  onChange={(event) => setPortalRecipientName(event.target.value)}
                />
              </div>
              <div className="col-12 col-md-5">
                <label className="form-label">بريد المستلم</label>
                <input
                  ...
                  onChange={(event) => setPortalRecipientEmail(event.target.value)}
                />
              </div>
              <div className="col-12 col-md-2">
                <button
                  ...
                  disabled={
                    Boolean(busy) ||
                    !portalRecipientName.trim() ||
                    !portalRecipientEmail.trim() ||
                    Number(portalDays) < 1 ||
                    Number(portalDays) > 30
                  }
                  onClick={() => setPortalDeliveryConfirmOpen(true)}
                >
                  إرسال الرابط
                </button>
              </div>
              ...
              <div className="border-top mt-3 pt-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
                <div>
                  ...
                  <div className="small aqua-muted">
                    72 ساعة بين الرسائل، وبحد أقصى 3 تذكيرات. التذكيرات الناجحة:{' '}
                    {invoice.contractAmendment.invoiceReminderCount}/3.
                  </div>
                  {invoice.contractAmendment.invoiceReminderScheduleEnabled &&
                  invoice.contractAmendment.invoiceReminderNextAt ? (
                    <div className="small text-info">
                      الجدولة مفعّلة — الموعد التالي:{' '}
                      <bdi dir="ltr">
                        {new Intl.DateTimeFormat('ar-JO-u-nu-latn', {
                          dateStyle: 'medium',
                          timeStyle: 'short',
                        }).format(new Date(invoice.contractAmendment.invoiceReminderNextAt))}
                      </bdi>
                    </div>
                  ) : null}
                  {invoice.contractAmendment.invoiceReminderFailedAt ? (
                    <div className="small text-warning">
                      فشلت المحاولة السابقة وبقي الرابط السابق فعالًا.
                    </div>
                  ) : null}
                </div>
                <div className="d-flex gap-2">
                  <button
                    ...
                    type="button"
                    disabled={Boolean(busy) || invoice.contractAmendment.invoiceReminderCount >= 3}
      ...
      canManage ? (
        <div className="aqua-card p-4 d-print-none">
          <h2 className="h5 fw-black mb-2">رابط الفاتورة العام</h2>
          <p className="aqua-muted">
            أنشئ رابطًا عشوائيًا محدود الصلاحية لهذه الفاتورة. إصدار رابط جديد يلغي الرابط السابق
            فورًا.
          </p>
          {invoice.publicIssuedAt && !invoice.publicRevokedAt ? (
            <AquaAlert variant="info" title="يوجد رابط مُصدر">
              ينتهي في {dateOnly(invoice.publicExpiresAt)} — عدد المشاهدات {invoice.publicViewCount}
              .
              {invoice.publicFirstViewedAt
                ? ` أول مشاهدة: ${dateOnly(invoice.publicFirstViewedAt)}.`
                : ' لم يُفتح بعد.'}
            </AquaAlert>
          ) : null}
          {linkPath ? (
            <div className="aqua-card-soft p-3 mt-3">
              <label className="form-label">الرابط الجديد — انسخه الآن</label>
              <div className="input-group" dir="ltr">
                <input className="form-control" readOnly value={linkUrl} />
                <button
                  className="btn btn-outline-info"
                  type="button"
          ...
          <div className="d-flex flex-wrap align-items-end gap-2 mt-3">
            ...
            <button
              className="btn btn-info fw-bold"
              type="button"
              disabled={Boolean(busy) || Number(linkDays) < 1 || Number(linkDays) > 30}
              onClick={() => manageLink('ISSUE')}
            >
              {busy === 'link-issue' ? 'جارٍ الإصدار...' : 'إصدار / تدوير الرابط'}
            </button>
            {invoice.publicIssuedAt && !invoice.publicRevokedAt ? (
              <button
                className="btn btn-outline-danger"
                type="button"
                disabled={Boolean(busy)}
                onClick={() => manageLink('REVOKE')}
              >
                {busy === 'link-revoke' ? 'جارٍ الإلغاء...' : 'إلغاء الرابط'}
              </button>
            ) : null}
          </div>
        </div>
      ...
      {canManage && ['ISSUED', 'PARTIALLY_PAID'].includes(invoice.status) && outstanding > 0 ? (
        <form className="aqua-card p-4 d-print-none" onSubmit={saveCollection}>
          <div className="d-flex flex-wrap justify-content-between gap-2 mb-3">
            <div>
              <h2 className="h5 fw-black mb-1">متابعة التحصيل</h2>
              <div className="small aqua-muted">كل حفظ ينشئ حدثًا زمنيًا في سجل النشاط.</div>
            </div>
            {invoice.collectionUpdatedAt ? (
              <div className="small aqua-muted">
                آخر تحديث: <bdi dir="ltr">{dateOnly(invoice.collectionUpdatedAt)}</bdi>
              </div>
            ) : null}
          </div>
          <div className="row g-3">
            <div className="col-12 col-md-4">
              ...
              >
                ...
                <option value="DISPUTED">اعتراض</option>
                <option value="ESCALATED">مصعّدة</option>
                <option value="CLOSED">مغلقة</option>
              </select>
            </div>
            <div className="col-12 col-md-4">
              <label className="form-label">مسؤول المتابعة</label>
              <select
                className="form-select"
            ...
            <div className="col-12 col-md-4">
              <label className="form-label">تاريخ الإجراء القادم</label>
              <AquaDatePicker value={collectionNextActionAt} onChange={setCollectionNextActionAt} />
            </div>
            <div className="col-12 col-md-8">
              <label className="form-label">الإجراء القادم</label>
              <input
                className="form-control"
                maxLength={500}
                value={collectionNextAction}
                onChange={(event) => setCollectionNextAction(event.target.value)}
              />
            </div>
            <div className="col-12 col-md-4">
              <label className="form-label">تاريخ وعد الدفع</label>
              <AquaDatePicker value={collectionPromiseDate} onChange={setCollectionPromiseDate} />
            </div>
            <div className="col-12">
              <label className="form-label">ملاحظات المتابعة</label>
              <textarea
                className="form-control"
                rows={3}
                maxLength={2000}
          ...
          <button className="btn btn-info fw-bold mt-3" type="submit" disabled={Boolean(busy)}>
            {busy === 'collection' ? 'جارٍ الحفظ...' : 'حفظ المتابعة'}
          </button>
        </form>
      ) : null}
        ...
        <form className="aqua-card p-4" onSubmit={saveDraft}>
          ...
          <div className="row g-3 mb-3">
            ...
            <div className="col-6 col-md-4">
              ...
              <input
                ...
                disabled={Boolean(invoice.contractAmendment)}
              ...
              <div className="row g-2 align-items-end aqua-card-soft p-2" key={index}>
                ...
                <div className="col-3 col-lg-2 d-grid">
                  <button
                    className="btn btn-outline-danger"
                    type="button"
                    disabled={items.length === 1 || Boolean(invoice.contractAmendment)}
                    onClick={() =>
          ...
          <div className="d-flex justify-content-between align-items-center gap-3 mt-4">
            <div className="fw-bold" dir="ltr">
              الإجمالي المتوقع: {money(preview.total, invoice.currency)}
            </div>
            <button className="btn btn-info fw-bold" disabled={Boolean(busy)} type="submit">
              {busy === 'save' ? 'جارٍ الحفظ...' : 'حفظ المسودة'}
            </button>
          </div>
        </form>
      ) : (
          ...
          <div className="aqua-card p-4 h-100">
            ...
            <div className="d-flex flex-column gap-3">
              <div className="d-flex justify-content-between">
                <span className="aqua-muted">المجموع الفرعي</span>
                <strong dir="ltr">{money(invoice.subtotal, invoice.currency)}</strong>
              </div>
              <div className="d-flex justify-content-between">
                <span className="aqua-muted">الخصم</span>
                <strong dir="ltr">{money(invoice.discountAmount, invoice.currency)}</strong>
              </div>
              <div className="d-flex justify-content-between">
                <span className="aqua-muted">الضريبة</span>
              ...
              <div className="d-flex justify-content-between text-warning">
                <span>المتبقي</span>
                <strong dir="ltr">{money(outstanding, invoice.currency)}</strong>
              </div>
            </div>
            <div className="small aqua-muted mt-4">
              الإصدار: {dateOnly(invoice.issueDate)} — الاستحقاق: {dateOnly(invoice.dueDate)}
            </div>
          </div>
        </div>
            ...
            outstanding > 0 ? (
              <form className="aqua-card-soft p-3 mb-4" onSubmit={recordPayment}>
                <div className="row g-2">
                  ...
                  <div className="col-6 col-lg-3">
                    <label className="form-label small">الطريقة</label>
                    <select
                      className="form-select"
                      value={paymentMethod}
                      onChange={(e) =>
                        setPaymentMethod(e.target.value as keyof typeof methodLabels)
                      }
                    >
                  ...
                  <div className="col-6 col-lg-3">
                    ...
                    <AquaDatePicker value={paymentDate} onChange={setPaymentDate} />
                  </div>
                  <div className="col-6 col-lg-3">
                    <label className="form-label small">المرجع</label>
                    <input
                      className="form-control"
                ...
                <button
                  ...
                  type="submit"
                >
                  {busy === 'payment' ? 'جارٍ التسجيل...' : 'تسجيل الدفعة'}
                </button>
              </form>
            ) : null}
                    ...
                    <div className="d-flex align-items-start justify-content-between gap-3">
                      <div>
                        <div
                          className={
                            payment.status === 'REVERSED'
                              ? 'text-decoration-line-through fw-bold'
                              : 'fw-bold'
                          }
                          dir="ltr"
                        >
                        ...
                        <div className="small aqua-muted mt-1">
                          {methodLabels[payment.method]} — {dateOnly(payment.paidAt)} —{' '}
                          {payment.recordedBy?.name ?? 'مستخدم محذوف'}
                        </div>
                        {payment.reference ? (
                      ...
                      {canManage && payment.status === 'POSTED' ? (
                        <div className="d-flex flex-wrap gap-2">
                          ...
                          <button
                            ...
                            disabled={Boolean(busy) || !invoice.client?.email}
                            type="button"
                            onClick={() => setReceiptPaymentId(payment.id)}
                          >
                            {busy === `receipt-${payment.id}` ? '...' : 'إرسال الإيصال'}
                          </button>
                          <button
                          ...
                          >
                            {busy === `reverse-${payment.id}` ? '...' : 'عكس'}
                          </button>
                        </div>
                      ) : (
                        <span
                          className={`badge ${payment.status === 'POSTED' ? 'text-bg-success' : 'text-bg-danger'}`}
                        >
                          {payment.status === 'POSTED' ? 'مرحّلة' : 'معكوسة'}
                        </span>
                      )}
                    </div>
                  </div>
      ...
      {invoice.notes || invoice.terms ? (
        <div className="row g-4">
          {invoice.notes ? (
            <div className="col-12 col-lg-6">
              <div className="aqua-card p-4 h-100">
                <h2 className="h6 fw-black">ملاحظات</h2>
                <p className="aqua-muted mb-0" style={{ whiteSpace: 'pre-wrap' }}>
                  {invoice.notes}
                </p>
              </div>
            </div>
          ...
          {invoice.terms ? (
            <div className="col-12 col-lg-6">
              <div className="aqua-card p-4 h-100">
                <h2 className="h6 fw-black">الشروط</h2>
                <p className="aqua-muted mb-0" style={{ whiteSpace: 'pre-wrap' }}>
                  {invoice.terms}
                </p>
              </div>
