...
import { AquaBadge, AquaCard, AquaDataPanel, AquaLinkButton } from '@/components/aqua';
...
function taskStatusVariant(status: string): AquaBadgeVariant {
  return (
    ({
      ...
      BLOCKED: 'danger',
      REVIEW: 'warning',
    }[status] as AquaBadgeVariant | undefined) ?? 'muted'
  );
}
...
export default async function DashboardPage() {
  ...
  const canViewCompanyProjects = hasRole(user.role, ACCESS_ROLES.projectManagement);
  const canViewServiceRequests = hasRole(user.role, ACCESS_ROLES.serviceRequestManagement);
  ...
  const canApproveExpenses = hasRole(user.role, ACCESS_ROLES.financeManagement);
  ...
  const [
    ...
    prisma.project.count({
      where: {
        companyId: user.companyId,
        status: { in: ['IN_PROGRESS', 'AT_RISK', 'IN_REVIEW', 'READY_FOR_DELIVERY'] },
        ...projectVisibility,
      },
    }),
    ...
    canApproveTime
      ? prisma.timesheet.count({
          where: {
            companyId: user.companyId,
            status: 'SUBMITTED',
            ...(user.role === 'OWNER' ? {} : { userId: { not: user.id } }),
          },
        })
      : Promise.resolve(null),
    canApproveLeave
    ...
    canApproveExpenses
      ? prisma.expense.count({
          where: {
            companyId: user.companyId,
            status: 'SUBMITTED',
            ...(user.role === 'OWNER' ? {} : { createdById: { not: user.id } }),
          },
        })
      : Promise.resolve(null),
  ]);
  ...
  const focusTasks = rawFocusTasks
    ...
    .sort((first, second) => {
      const firstBlocked = first.status === 'BLOCKED' || first.blockers.length > 0;
      const secondBlocked = second.status === 'BLOCKED' || second.blockers.length > 0;
      ...
      const priorityDifference =
        (priorityRank[first.priority] ?? 4) - (priorityRank[second.priority] ?? 4);
  ...
  const metrics: DashboardMetric[] = [
    ...
    {
      label: canViewCompanyProjects ? 'مشاريع قيد التنفيذ' : 'مشاريعي الجارية',
        ...
        : [
            {
              ...
              icon: Inbox,
              variant: newServiceRequestCount > 0 ? ('aqua' as const) : ('success' as const),
            },
          ]),
      ...(canViewSales
        ? [
            {
              ...
              icon: PhoneCall,
              variant: overdueSalesFollowUps > 0 ? ('warning' as const) : ('success' as const),
            },
          ]
        : []),
        ...
        : [
            {
              ...
              variant: overdueInvoiceCount > 0 ? ('danger' as const) : ('success' as const),
            },
          ]),
  ...
  const activeAttentionPaths = attentionItems.filter((item) => item.value > 0).length;
    ...
    <div className="aqua-dashboard-overview">
      ...
      <div className="aqua-dashboard-workspace">
        <AquaDataPanel
          title="تركيز اليوم"
          meta={
            <AquaBadge variant={focusTasks.length > 0 ? 'warning' : 'success'} size="sm">
              {focusTasks.length > 0 ? `${focusTasks.length} ظاهرة` : 'لا مهام عاجلة'}
            </AquaBadge>
          }
        >
          {focusTasks.length === 0 ? (
            ...
            <ol className="aqua-dashboard-focus-list">
              {focusTasks.map((task) => {
                const blocked = task.status === 'BLOCKED' || task.blockers.length > 0;
                ...
                return (
                  <li key={task.id} className="aqua-dashboard-focus-item">
                    ...
                    <div className="aqua-dashboard-focus-item__copy">
                      ...
                      <span>{task.project?.name ?? 'مهمة تشغيلية غير مرتبطة بمشروع'}</span>
                    </div>

                    <div className="aqua-dashboard-focus-item__badges">
                      <AquaBadge variant={taskStatusVariant(task.status)} size="sm">
                        {taskStatusLabel(task.status)}
                      </AquaBadge>
                      <AquaBadge variant={taskDueVariant(task.bucket)} size="sm">
                        {taskDueLabel(task.bucket, task.dueDate)}
                        {task.dueDate ? ` • ${formatTaskDueDate(task.dueDate, timeZone)}` : ''}
                      </AquaBadge>
                    </div>
                  </li>
                );
        ...
        <AquaDataPanel
          ...
          meta={
            <AquaBadge variant={activeAttentionPaths > 0 ? 'warning' : 'success'} size="sm">
            ...
            <div className="aqua-dashboard-attention-list">
              {attentionItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className="aqua-dashboard-attention-item__icon" aria-hidden="true">
                      <AttentionIcon />
                    </span>
      ...
      <AquaDataPanel
        ...
        actions={
          canViewCompanyActivity ? (
            <AquaLinkButton href="/dashboard/activity" variant="ghost" size="sm">
              السجل
            </AquaLinkButton>
          ) : null
        }
          ...
          <ol className="aqua-dashboard-activity-list">
            {recentActivities.map((activity) => (
              <li key={activity.id} className="aqua-dashboard-activity-item">
                <span className="aqua-dashboard-activity-item__icon" aria-hidden="true">
                  <Activity />
                </span>
