...
function actionLabel(action: ActivityAction) {
  const labels: Record<ActivityAction, string> = {
    ...
    PROJECT_AMENDMENT_INVOICE_DELIVERY_PREPARED: 'تجهيز إرسال فاتورة ملحق',
    PROJECT_AMENDMENT_INVOICE_DELIVERY_FAILED: 'فشل إرسال فاتورة ملحق',
    PROJECT_AMENDMENT_INVOICE_SENT: 'إرسال فاتورة ملحق للعميل',
    PROJECT_AMENDMENT_INVOICE_PORTAL_ISSUED: 'إصدار بوابة فاتورة العميل',
    PROJECT_AMENDMENT_INVOICE_PORTAL_REVOKED: 'إلغاء بوابة فاتورة العميل',
    PROJECT_AMENDMENT_INVOICE_PORTAL_VIEWED: 'فتح بوابة فاتورة العميل',
    PROJECT_AMENDMENT_INVOICE_PORTAL_DELIVERY_PREPARED: 'تجهيز إرسال بوابة الفاتورة',
    PROJECT_AMENDMENT_INVOICE_PORTAL_DELIVERY_FAILED: 'فشل إرسال بوابة الفاتورة',
    PROJECT_AMENDMENT_INVOICE_PORTAL_SENT: 'إرسال بوابة الفاتورة للعميل',
    PROJECT_AMENDMENT_INVOICE_REMINDER_PREPARED: 'تجهيز تذكير دفع فاتورة',
    PROJECT_AMENDMENT_INVOICE_REMINDER_FAILED: 'فشل تذكير دفع فاتورة',
    PROJECT_AMENDMENT_INVOICE_REMINDER_SENT: 'إرسال تذكير دفع فاتورة',
    PROJECT_AMENDMENT_INVOICE_REMINDER_SCHEDULE_ENABLED: 'تفعيل جدولة تذكيرات فاتورة',
    PROJECT_AMENDMENT_INVOICE_REMINDER_SCHEDULE_DISABLED: 'إيقاف جدولة تذكيرات فاتورة',
    PROJECT_AMENDMENT_INVOICE_SCHEDULED_REMINDER_SENT: 'إرسال تذكير فاتورة مجدول',
    PROJECT_AMENDMENT_INVOICE_SCHEDULED_REMINDER_FAILED: 'فشل تذكير فاتورة مجدول',
...
export default async function ActivityPage({
  ...
  const [totalActivities, todayActivities, loginCount, teamChanges] = await Promise.all([
    ...
    prisma.activityLog.count({
      where: {
        companyId: user.companyId,
        action: {
          in: ['USER_CREATED', 'USER_UPDATED', 'USER_DEACTIVATED', 'USER_ACTIVATED'],
        },
      },
    }),
  ]);
  ...
  return (
    <div className="aqua-compact-page aqua-activity-page aqua-admin-governance-page">
      ...
      <div className="aqua-card aqua-log-table-card aqua-admin-panel p-4">
        ...
        {activities.length === 0 ? (
          <div className="aqua-card-soft p-5 text-center aqua-soft">لا توجد نشاطات حتى الآن.</div>
        ) : (
          <>
                ...
                <tbody>
                  {activities.map((activity, index) => (
                    <tr key={activity.id}>
                      ...
                      <td>
                        <div className="fw-bold">{activity.user?.name || 'System'}</div>
