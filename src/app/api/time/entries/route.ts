...
// Safety net, not pagination: this endpoint is a single-week view meant to
// render whole, so it caps rather than paginates — see docs/BATCH_PAGINATION_01.md.
const MAX_WEEK_ENTRIES = 1000;
...
export async function POST(request: Request) {
  try {
    ...
    if (parsed.data.workDate > localDateKey(new Date(), user.company.timezone)) {
      throw new ApiError('لا يمكن تسجيل ساعات على تاريخ مستقبلي', 400, 'FUTURE_TIME_ENTRY');
    }
    ...
    return handleApiError(error, 'TIME_ENTRIES_POST_ERROR', 'تعذر تسجيل ساعات العمل');
  }
}
