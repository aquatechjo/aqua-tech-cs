...
const updateTaskSchema = z.object({
  ...
  source: z.enum(['MANUAL', 'WEBSITE_REQUEST', 'WORKFLOW', 'AI_GENERATED']).optional(),
...
async function getTask(_request: Request, context: { params: Promise<{ id: string }> }) {
...
async function updateTask(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let safeClientId = data.clientId !== undefined ? data.clientId || null : existingTask.clientId;
  const safeAssignedToId =
    data.assignedToId !== undefined ? data.assignedToId || null : existingTask.assignedToId;
  const projectChanged = data.projectId !== undefined && safeProjectId !== existingTask.projectId;
  const clientChanged = data.clientId !== undefined && safeClientId !== existingTask.clientId;

  if (projectChanged && !canUseTaskProject(scope, safeProjectId)) {
    throw new ApiError(
  ...
  if (safeProjectId) {
    ...
    if (
      projectExecutionNeedsActivation({
        assignedToId: data.assignedToId !== undefined || projectChanged ? safeAssignedToId : null,
  ...
  } else if (!scope.canViewCompanyTasks && safeClientId && (clientChanged || projectChanged)) {
  ...
  const task = await prisma.$transaction(async (tx) => {
    const updatedTask = await tx.task.update({
      ...
      data: {
        ...
        ...(data.sourceRef !== undefined ? { sourceRef: nullableText(data.sourceRef) } : {}),
        ...(data.estimatedHours !== undefined
          ? { estimatedHours: nullableDecimal(data.estimatedHours) }
          : {}),
        ...(data.dueDate !== undefined ? { dueDate: nullableDate(data.dueDate) } : {}),
        progress: nextProgress,
        ...(nextStatus === 'DONE' && !existingTask.completedAt ? { completedAt: new Date() } : {}),
        ...(nextStatus !== 'DONE' ? { completedAt: null } : {}),
        ...(!['TODO', 'CANCELLED', 'ARCHIVED'].includes(nextStatus) && !existingTask.startedAt
          ? { startedAt: new Date() }
          : {}),
    ...
    if (data.projectId !== undefined && safeProjectId && existingTask.participants.length > 0) {
...
export const PATCH = withApiHandler('TASK_PATCH_ERROR', updateTask, 'حدث خطأ أثناء تعديل المهمة');
