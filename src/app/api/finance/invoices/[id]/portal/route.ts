...
async function manage(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (parsed.data.action === "ISSUE") {
      ...
      await tx.projectContractAmendment.update({ where: { id: invoice.contractAmendment.id }, data: { invoicePortalTokenHash: access.tokenHash, invoicePortalExpiresAt: access.expiresAt, invoicePortalIssuedAt: now, invoicePortalRevokedAt: null, invoicePortalFirstViewedAt: null, invoicePortalLastViewedAt: null, invoicePortalViewCount: 0 } })
    ...
    await tx.projectContractAmendment.update({ where: { id: invoice.contractAmendment.id }, data: { invoicePortalTokenHash: null, invoicePortalExpiresAt: null, invoicePortalRevokedAt: now } })
